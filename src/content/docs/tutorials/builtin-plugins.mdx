...
description: "Learn from Hytale's 31 built-in plugin packages that demonstrate modding patterns and provide core gameplay features."
---
...
All 31 built-in plugin packages organized by category:
...
`RandomTickPlugin` was added in Update 3. It provides Minecraft-style random block ticking with built-in `ChangeIntoBlock` and `SpreadTo` procedures. See [Block Ticking](/plugin-development/blocks/block-ticking/) for details. All Update 3 additions are listed in the [changelog](/changelog/update-2-to-3/).
</Aside>
