...
description: "Learn ECS architecture that separates data (components) from behavior (systems) for efficient entity management."
---
...
flowchart TB
    subgraph Store
        subgraph Entities["Entities (Refs)"]
            E0["Entity Ref 0"]
            E1["Entity Ref 1"]
            E2["Entity Ref 2"]
            E3["Entity Ref 3"]
        ...
        subgraph Resources["Resources (World Data)"]
            TimeOfDay
            Weather
            GameRules
        end
...
| `EntityHolderEventSystem` | Handles events with entity holder context |
...
`EntityHolderEventSystem` is a new system type that handles events dispatched on entity holders (via `EntityHolderEventType`). Unlike `EntityEventSystem` which receives an archetype chunk index, this system receives a `Holder` directly.
</Aside>

```java
import com.hypixel.hytale.component.system.EntityHolderEventSystem;

public class MyHolderEventSystem
        extends EntityHolderEventSystem<ChunkStore, BlockReplaceEvent> {

    public MyHolderEventSystem() {
        super(BlockReplaceEvent.class);
    }

    @Override
    public void handle(Holder<ChunkStore> holder, Store<ChunkStore> store,
                       CommandBuffer<ChunkStore> commandBuffer,
                       BlockReplaceEvent event) {
        // handle replacement
    }
...
`Ref` no longer overrides `equals()` and `hashCode()` — refs now use identity comparison (object reference equality) instead of value comparison. A `validate(Store)` method was added that checks both store identity and index validity. Details and other breaking changes are covered in the [Update 3 changelog](/changelog/update-2-to-3/).
</Aside>

<Aside type="caution" title="Changed in Update 4">
`Ref.validate(Store)` now returns `int` (the entity index) instead of `void`. Collections holding `Ref` instances should use `ReferenceArrayList` and `ReferenceOpenHashSet` (identity-based) instead of `ObjectArrayList` and `HashSet`. `SpatialResource.getThreadLocalReferenceList()` now returns `List<Ref<ECS_TYPE>>` instead of `ObjectList<Ref<ECS_TYPE>>`.
</Aside>
...
if (ref.isValid()) {
    // Get component (returns null if entity doesn't have it)
    HealthComponent health = store.getComponent(ref, healthType);

    // Check if entity has component via archetype
    if (store.getArchetype(ref).contains(healthType)) {
        // ...
    }
}
```
...
5. **Use constructor injection for ComponentTypes**: Pass component types to systems via constructor parameters rather than storing them as static fields
6. **Check ref validity**: Always check `ref.isValid()` before use
...
public class DamageOverTimeSystem extends EntityTickingSystem<EntityStore> {
    ...
    public void tick(float dt, int index, ArchetypeChunk<EntityStore> chunk,
        ...
        if (dot.getDuration() <= 0) {
            ...
            commandBuffer.removeComponent(chunk.getReferenceTo(index), dotType);
        }
    }
}
```
...
- [Plugin Registries](/plugin-development/core-concepts/registries/) - ECS registration methods
- [Entity System](/plugin-development/entities/overview/) - Entity types and spawning
