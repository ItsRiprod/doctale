...
**See also:** [Command System](/plugin-development/core-concepts/commands/)
...
**See also:** [Event System](/plugin-development/core-concepts/event-system/)
...
**See also:** [ECS Overview](/plugin-development/core-concepts/ecs-overview/)
...
**See also:** [Asset System](/asset-development/overview/)
...
```java
// isEnabled() returns true during SETUP, START, and ENABLED states
if (isEnabled()) {
    // Plugin is active
}

// isDisabled() returns true during NONE, SHUTDOWN, and DISABLED states
if (isDisabled()) {
    // Plugin has not started, is shutting down, or has been disabled
}
...
<Aside type="note" title="New in Update 3">
`Registry` now has a `shutdownAndCleanup(boolean)` method that disables the registry, iterates registrations in reverse (LIFO) order calling cleanup on each, then clears the list. This formalizes the shutdown lifecycle for registries. More details in the [Update 3 changelog](/changelog/update-2-to-3/).
</Aside>
