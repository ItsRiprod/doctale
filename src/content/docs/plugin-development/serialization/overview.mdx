...
- **Schema generation** - Automatic JSON schema output via `SchemaGenerator`
...
`GenerateSchemaEvent` was removed. Schema generation is now handled by `SchemaGenerator` at `com.hypixel.hytale.server.core.schema.SchemaGenerator`. Plugins that registered schemas via event handlers should use `SchemaGenerator.registerAssetSchema()` or `SchemaGenerator.registerConfig()` instead. The `StateData` codec also changed: `getId()` was removed, `DEFAULT_CODEC_BUILDER` was renamed to `CODEC_BUILDER`, and `DEFAULT_CODEC` was renamed to `CODEC`.
</Aside>
