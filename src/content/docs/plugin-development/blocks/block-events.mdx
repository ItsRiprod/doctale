...
`BlockReplaceEvent` is a new ECS event at `com.hypixel.hytale.server.core.modules.block.BlockReplaceEvent`, dispatched on the `ChunkStore` when a block entity is being replaced. It provides the chunk reference, block position, and the new entity holder. Handle it via `EntityEventSystem<ChunkStore, BlockReplaceEvent>` or `EntityHolderEventSystem<ChunkStore, BlockReplaceEvent>`.
</Aside>
