...
description: "Apply status effects with duration, stacking, stat modifiers, and visual feedback through EffectControllerComponent."
---
...
public class EffectControllerComponent implements Component<EntityStore> {
    // Add effect using its asset defaults (duration, overlap, infinite)
    ...
    // Add effect with explicit index, using its asset defaults
    public boolean addEffect(
        Ref<EntityStore> ownerRef,
    ...
    // Add effect with explicit index, custom duration and overlap behavior
    public boolean addEffect(
        Ref<EntityStore> ownerRef,
        int entityEffectIndex,
        EntityEffect entityEffect,
...
for (int index : indices) {
    ...
    if (active.isInfinite()) {
        ...
        // Effect has duration
        System.out.println("Effect " + effectIndex + ": " + remaining + "s remaining");
    }
}
```
...
`EntityStatType` gained a `hideFromTooltip` boolean field in Update 3, allowing stat types to be hidden from the UI tooltip display. Browse the [Update 3 changelog](/changelog/update-2-to-3/) for all entity-related updates.
</Aside>

<Aside type="note" title="New in Update 4">
`EntityEffect` gained an `applyConditions` field — an array of `Condition` objects that must all evaluate to `true` for the effect to remain active. If any condition fails, the effect is automatically removed. The condition system was also relocated from `com.hypixel.hytale.server.core.modules.entitystats.asset.condition` to `com.hypixel.hytale.server.core.modules.entity.condition`. New condition types added in Update 4 include `CheckPlayerGameModeCondition`, `HasEffectCondition`, `InFluidCondition`, and `IsPlayerCondition`.
</Aside>
...
- [Knockback System](/plugin-development/entities/knockback/) - Knockback mechanics
