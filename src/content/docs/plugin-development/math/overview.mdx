...
Five custom math types were removed and replaced by JOML equivalents:

| Removed Type | JOML Replacement |
|---|---|
| `com.hypixel.hytale.math.Mat4f` | `org.joml.Matrix4f` / `Matrix4fc` |
| `com.hypixel.hytale.math.Vec2f` | `org.joml.Vector2f` / `Vector2fc` |
| `com.hypixel.hytale.math.Vec3f` | `org.joml.Vector3f` / `Vector3fc` |
| `com.hypixel.hytale.math.Vec4f` | `org.joml.Vector4f` / `Vector4fc` |
| `com.hypixel.hytale.math.Quatf` | `org.joml.Quaternionf` / `Quaternionfc` |

The JOML dependency is declared in `Protocol/pom.xml`. All ByteBuf serialization logic previously in these types has been moved to `PacketIO` methods. JOML types use `*fc` read-only interface variants for parameters where immutability is desired. The Hytale-specific vector types (`Vector3d`, `Vector3f`, `Vector3i`, `Vector2i`, etc. in `com.hypixel.hytale.math.vector`) are **not** affected.
</Aside>
...
`Vector3d.assign(Vector3i)` was added to assign from an integer vector.
</Aside>
...
<Aside type="note" title="New in Update 4">
`ChunkUtil` gained chunk coordinate validation methods and constants.
</Aside>
...
public class ChunkUtil {
    ...
    public static boolean isValidChunkIndex(long index);
    public static boolean isValidChunkCoords(int x, int z);
    public static boolean isValidChunkCoord(int coord);
}
```
