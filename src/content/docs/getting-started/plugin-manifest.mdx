---
title: "Plugin Manifest (manifest.json)"
description: "Configure your plugin's identity, version, dependencies, and entry point in the required manifest.json manifest file."
---
...
The version of your plugin following [Semantic Versioning 2.0.0](https://semver.org/) format. This string is parsed into a `Semver` object internally.
...
`ServerVersion` is now stored as a plain `String` instead of a parsed `SemverRange`. The `getServerVersion()` method on `PluginManifest` returns `String` instead of `SemverRange`. A `setServerVersion(@Nullable String)` setter was also added. Version validation was changed from semver range comparison to a string equality check in the `PluginManager`. The [Update 3 changelog](/changelog/update-2-to-3/) covers all manifest-related changes.
</Aside>
...
<Aside type="caution" title="Wildcard behavior">
Using `"*"` or omitting `ServerVersion` entirely will log a warning: *"Plugin does not specify a target server version. You may encounter issues, please check for plugin updates. This will be a hard error in the future"*. Always set an exact version string for production plugins.
</Aside>

<Aside type="caution" title="Changed in Update 4">
The `shouldSkipModValidation()` method and the `skipModValidationForVersion` config field on `HytaleServerConfig` have been removed. Mod validation now uses `assetPack.isImmutable()` combined with the `IGNORE_BROKEN_MODS` server option instead.
</Aside>
...
The server now supports multiple mods directories via the `MODS_DIRECTORIES` option. Plugins can be loaded from additional directories beyond the default `plugins/` folder.
</Aside>

## Related

- [Plugin Lifecycle](/getting-started/plugin-lifecycle/) - Understanding plugin states and lifecycle methods
- [JavaPlugin Class](/getting-started/plugin-lifecycle/) - The main plugin base class
